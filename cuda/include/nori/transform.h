...
public:
    ...
    /// Create a new transform instance for the given matrix 
     __host__  Transform(const Eigen::Matrix4f &trafo);

    /// Create a new transform instance for the given matrix and its inverse
    __device__ __host__  Transform(const Eigen::Matrix4f &trafo, const Eigen::Matrix4f &inv)
        : m_transform(trafo), m_inverse(inv) { }

    /// Return the underlying matrix
    __device__ __host__  const Eigen::Matrix4f &getMatrix() const {
        return m_transform;
    }

    /// Return the inverse of the underlying matrix
    __device__ __host__  const Eigen::Matrix4f &getInverseMatrix() const {
        return m_inverse;
    }
    ...
    __device__ __host__  Ray3f operator*(const Ray3f &r) const {
        return Ray3f(
            operator*(r.o), 
            operator*(r.d), 
            r.mint, r.maxt
