...
struct BSDFQueryRecord {
    ...
    /// difference to point of outgoing direction (in local frame at xi)
    Vector2f dx;

    // outgoing intersection defining (xo = its_out.p)
    Intersection its_out;

    Ray3f probeRay;
    bool probe = false;
    float dxPdf = 1.0f;
    float dxAxisPdf = 1.0f;
	...
	bool isDelta = false; // last sample is delta
};
...
public:
     ...
     *         failed.
     */
    virtual Color3f sample(BSDFQueryRecord &bRec, const Intersection& its, const Point2f &sample1, const Point2f &sample2) const = 0;

    virtual void sampleDx(BSDFQueryRecord &bRec, const Intersection& its, Sampler& sampler) const { bRec.its_out = its; }
