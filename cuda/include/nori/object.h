...
public:
    ...
    /**
     * Returns the size of this object in bytes. Needs to be overitten by all subclasses.
     * The value is then used to copy the object over to GPU memory
     */
    virtual size_t getSize() const   {return sizeof(NoriObject);}
     /**
     * \brief As when copying the object to GPU all pointers will get invalid the object. This function needs to be implemented
     *  by every Object which uses pointers.This method will be called from CPU and should be the last object function called from CPU,
     *  because pointer should be invalid afterwards
     * \param objects list of all gpu memory locations, to which the NoriObject will be
     * copied to gpu (additionally  objects[oID] points to the gpu location of the corropondending nori object)
     */
    virtual void gpuTransfer(NoriObject **objects) {/* Do nothing*/}

    /**
     * Following lines are layed out such that we get two things a linked list of all CUDA Objects in memory
     * A unique objectId for every NoriObject which is continuous (staring from 0 see object.cpp)
     */
    /**
     * uniqe id for the object
     */
    static unsigned int count;
#ifdef __CUDA_ARCH__
    unsigned int objectId = 0;
#else
    unsigned int objectId  = count++;
#endif
    /**
     *
     * Arguably not the nicest way to setup a linked list. Also not thread safe.
     */
    static NoriObject* head;
    static NoriObject* current;
    /**
     * C comma operator :1.set head if no head is defined
     *                   2.link this element to the previous one
     *                   3.update current
     *                   4. set the value of the whole expression tio NULL
     */
#ifdef __CUDA_ARCH__
    NoriObject *next = nullptr;
#else
    NoriObject *next = (!head&&(head=this),(current&&(current->next=this),NoriObject::current = this), (NoriObject*)nullptr);
#endif
    enum EClassType {
        EScene = 0,
        EMesh,
    ...
    static std::string classTypeName(EClassType type) {
        switch (type) {
            ...
            case ENormalModifier:    return "normalModifier";
            case ETest:       return "test";
            case EEnvMap:      return "enviromentMap";
    ...
    /// Virtual destructor
    virtual ~NoriObject() {
        std::cout << "NoriObject destructed : " << this->toString() << std::endl;
    ...
    virtual std::string toString() const {return "NoString";};
