...
public:
    virtual size_t getSize() const override {return sizeof(Mesh);}
    WavefrontOBJ(const PropertyList &propList) {
        typedef std::unordered_map<OBJVertex, uint32_t, OBJVertexHash> VertexMap;
        auto file = propList.getString("filename");
        filesystem::path filename =
            getFileResolver()->resolve(file);
        ...
        void *p = malloc(indices.size()*sizeof(uint32_t));
        new(&m_F) Eigen::Map<MatrixXu>((uint32_t  *)p,3,indices.size()/3);
        memcpy(m_F.data(), indices.data(), sizeof(uint32_t)*indices.size());

        p = malloc(3*vertices.size()*sizeof(float));
        new(&m_V) Eigen::Map<MatrixXf>((float *)p,3,vertices.size());
        ...
        if (!normals.empty()) {
            ...
            p = malloc(3*vertices.size()*sizeof(float));
            new(&m_N) Eigen::Map<MatrixXf>((float  *)p,3,vertices.size());
        ...
        if (!texcoords.empty()) {
            ...
            p = malloc(2*vertices.size()*sizeof(float));
            new(&m_UV) Eigen::Map<MatrixXf>((float  *)p,2,vertices.size());
            for (uint32_t i=0; i<vertices.size(); ++i)
