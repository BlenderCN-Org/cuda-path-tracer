...
public:
    ...
    std::pair<Point3f, float> warpPoint(WarpType warpType, const Point2f &sample, float parameterValue) {
        ...
        switch (warpType) {
            ...
            case DiskExpDecay: result << Warp::squareToExpDecayDisk(parameterValue, sample), 0; break;
            case DiskSubsurface: result << Warp::squareToSubsurfaceRd(parameterValue, sample), 0; break;
            ...
            case MicrofacetBRDF: {
                ...
                //Intersection its;
                //float value = m_brdf->sample(bRec, its, sample, sample).getLuminance();
                float value = 0;
    ...
    void refresh() {
        ...
        m_parameterSlider->setEnabled(warpType == Beckmann || warpType == MicrofacetBRDF || warpType == UniformSphereCap 
				|| warpType == DiskExpDecay 
				|| warpType == DiskSubsurface);
    ...
    void drawContents() {
        ...
        if (m_drawHistogram) {
            ...
            const int histHeight = (warpType == None || warpType == Disk 
					|| warpType == DiskExpDecay 
					|| warpType == DiskSubsurface) ? histWidth : histWidth / 2;
    ...
    void runTest() {
        ...
        if (warpType != None && warpType != Disk && warpType != DiskExpDecay && warpType != DiskSubsurface)
            xres *= 2;
        ...
        for (int i=0; i<sampleCount; ++i) {
            ...
            if (warpType == None) {
                x = sample.x();
                y = sample.y();
            } else if (warpType == Disk || warpType == DiskExpDecay || warpType == DiskSubsurface) {
        ...
        auto integrand = [&](double y, double x) -> double {
            ...
            } else if (warpType == DiskExpDecay) {
                x = x * 2 - 1; y = y * 2 - 1;
                return Warp::squareToExpDecayDiskPdf(parameterValue, Point2f(x, y));
            } else if (warpType == DiskSubsurface) {
                x = x * 2 - 1; y = y * 2 - 1;
                return Warp::squareToSubsurfaceRdPDF(parameterValue, Point2f(x, y));
            } else {
                x *= 2 * M_PI;
                y = y * 2 - 1;
    ...
    void initializeGUI() {
        ...
        m_warpTypeBox = new ComboBox(m_window, { "None", "Disk", "DiskExp", "Subsurface", "Sphere", "Spherical cap", "Hemisphere (unif.)",
