            ...
            for (auto bsdf : m_bsdfs) {
                for (size_t i=0; i<m_references.size(); ++i) {
                    ...
                    for (int k=0; k<m_sampleCount; ++k) {
                        Point2f sample1(random.nextFloat(), random.nextFloat());
                        Point2f sample2(random.nextFloat(), random.nextFloat());
                        double result = (double) bsdf->sample(bRec, Intersection(), sample1, sample2).getLuminance();
