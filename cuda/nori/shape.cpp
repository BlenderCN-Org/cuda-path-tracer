...
void Shape::addChild(NoriObject *obj) {
    switch (obj->getClassType()) {
        ...
        case ENormalModifier:
            if (m_normalModifier)
                throw NoriException(
                    "Shape: tried to register multiple Normal modifier instances!");
            m_normalModifier = static_cast<NormalModifier *>(obj);
            break;
    ...
    void Shape::gpuTransfer(NoriObject **objects) {
        m_bsdf = (BSDF *) objects[m_bsdf->objectId];
        if(m_emitter)
            m_emitter = (Emitter *) objects[m_emitter->objectId];
        if(m_normalModifier)
            m_normalModifier = (NormalModifier *) objects[m_normalModifier->objectId];
    }
