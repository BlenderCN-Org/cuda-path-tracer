...
public:
    ...
    /// Uniformly sample a vector on a 2D disk with radius 1, centered around the origin
    static Point2f squareToExpDecayDisk(float sigma, const Point2f &sample);

    /// Probability density of \ref squareToUniformDisk()
    static float squareToExpDecayDiskPdf(float sigma, const Point2f &p);

    /// Uniformly sample a vector on a 2D disk with radius 1, centered around the origin
    static Point2f squareToSubsurfaceRd(float sigma, const Point2f &sample);

    /// Probability density of \ref squareToUniformDisk()
    static float squareToSubsurfaceRdPDF(float sigma, const Point2f &p);
