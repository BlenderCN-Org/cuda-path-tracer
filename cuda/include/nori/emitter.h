...
struct EmitterQueryRecord {
    ...
    __device__ __host__ EmitterQueryRecord(const Point3f &ref) : ref(ref) { }

    /**
     * \brief Create a query record that can be used to query the
     * sampling density after having intersected an area emitter
     */
    __device__ __host__ EmitterQueryRecord(const Point3f &ref, const Point3f &p, const Normal3f &n) :
        ref(ref), p(p), n(n) {
        wi = (p - ref).normalized();
    }
};
...
public:
    ...
    virtual size_t getSize() const  override  {return sizeof(Emitter);}
    enum emitterType {EArea,EEnv} emitterType;
    /**
    ...
    virtual void gpuTransfer(NoriObject **objects)  override {m_shape = (Shape *) objects[m_shape->objectId]; };
...
#define CallEmitter(object,function,...) CallCuda1(nori::Emitter,emitterType,object,function,const,EArea,AreaEmitter,__VA_ARGS__)
NORI_NAMESPACE_END
