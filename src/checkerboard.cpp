...
public:
    ...
    virtual T eval(const Point2f & uv) override {
		...
		// take modulo
		float modx = fmod(x, 2.0f);
		float mody = fmod(y, 2.0f);

		// extra care of negative values: to get the pattern we want, we need to shift the coordinates by 1
		if (x < 0) modx = fmod(fabs(x)+1, 2.0f);
		if (y < 0) mody = fmod(fabs(y)+1, 2.0f);

		// pattern in [0,2], [0,2] coordinate frame
