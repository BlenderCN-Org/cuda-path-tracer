...
template <typename _PointType> struct TBoundingBox {
    ...
    const __device__ __host__ bool rayIntersect(const Ray3f &ray) const {
        //This function does not work with less or unequal to 3
        //if(Dimension != 3)
        //   return false;

        PointType t1 = ray.dRcp.cwiseProduct(min - ray.o) ;
        PointType t2 = ray.dRcp.cwiseProduct(max - ray.o) ;
        float nearT = fmaxf(fminf(t1(0),t2(0)),fmaxf(fminf(t1(1),t2(1)),fminf(t1(2),t2(2))));
        float farT =  fminf(fmaxf(t1(0),t2(0)),fminf(fmaxf(t1(1),t2(1)),fmaxf(t1(2),t2(2))));
        //printf("%f\n",farT);

        return  (nearT<=farT)&&ray.mint <= farT && nearT <= ray.maxt ;
    }
