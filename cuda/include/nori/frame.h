...
struct Frame {
    ...
    __device__ __host__  Frame() { }

    /// Given a normal and tangent vectors, construct a new coordinate frame
    __device__ __host__  Frame(const Vector3f &s, const Vector3f &t, const Normal3f &n)
     : s(s), t(t), n(n) { }

    /// Construct a frame from the given orthonormal vectors
    __device__ __host__  Frame(const Vector3f &x, const Vector3f &y, const Vector3f &z)
     : s(x), t(y), n(z) { }

    /// Construct a new coordinate frame from a single vector
    __device__ __host__  Frame(const Vector3f &n) : n(n) {
        coordinateSystem(n, s, t);
    }

    /// Convert from world coordinates to local coordinates
    __device__ __host__ Vector3f toLocal(const Vector3f &v) const {
        return Vector3f(
    ...
    __device__ __host__ Vector3f toWorld(const Vector3f &v) const {
        return s * v.x() + t * v.y() + n * v.z();
    }

    /** \brief Assuming that the given direction is in the local coordinate 
     * system, return the cosine of the angle between the normal and v */
    __device__ __host__  static float cosTheta(const Vector3f &v) {
        return v.z();
    }

    /** \brief Assuming that the given direction is in the local coordinate
     * system, return the sine of the angle between the normal and v */
    __device__ __host__  static float sinTheta(const Vector3f &v) {
    ...
    __device__ __host__  static float tanTheta(const Vector3f &v) {
    ...
    __device__ __host__  static float sinTheta2(const Vector3f &v) {
        return 1.0f - v.z() * v.z();
    }

    /** \brief Assuming that the given direction is in the local coordinate 
     * system, return the sine of the phi parameter in spherical coordinates */
    __device__ __host__  static float sinPhi(const Vector3f &v) {
    ...
    __device__ __host__  static float cosPhi(const Vector3f &v) {
        float sinTheta = Frame::sinTheta(v);
        if (sinTheta == 0.0f)
            return 1.0f;
     ...
     * coordinates */
    __device__ __host__  static float sinPhi2(const Vector3f &v) {
     ...
     * coordinates */
    __device__ __host__  static float cosPhi2(const Vector3f &v) {
        return clamp(v.x() * v.x() / sinTheta2(v), 0.0f, 1.0f);
    }

    /// Equality test
     __host__  bool operator==(const Frame &frame) const {
    ...
    __device__ __host__  std::string toString() const {
        return tfm::format(
                "Frame[\n"
                "  s = %s,\n"
                "  t = %s,\n"
