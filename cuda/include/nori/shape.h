...
struct Intersection {
    ...
    __device__ Intersection() : mesh(nullptr) { }

    /// Transform a direction vector into the local shading frame
    __device__ Vector3f toLocal(const Vector3f &d) const {
        return shFrame.toLocal(d);
    }

    /// Transform a direction vector from local to world coordinates
    __device__ Vector3f toWorld(const Vector3f &d) const {
...
struct ShapeQueryRecord {
    ...
    /// Empty constructor
    __device__ ShapeQueryRecord() {}
    /// Data structure with ref to call sampleSurface()
    __device__ ShapeQueryRecord(const Point3f & ref_) : ref(ref_) {}
    /// Data structure with ref and p to call pdfSurface()
    __device__  ShapeQueryRecord(const Point3f & ref_, const Point3f & p_) : ref(ref_), p(p_) {}
...
public:
    ...
    virtual size_t getSize() const  override  {return sizeof(Shape);}
    /// Release all memory
    virtual ~Shape();
    enum shapeType {ETMesh,ETSphere} shapeType;
    ...
    __device__ __host__ const BoundingBox3f &getBoundingBox() const { return m_bbox; }

    /// Is this mesh an area emitter?
    __device__ __host__ bool isEmitter() const { return m_emitter != nullptr; }

    /// Return a pointer to an attached area emitter instance
   __device__ Emitter *getEmitter() { return m_emitter; }

    /// Return a pointer to an attached area emitter instance (const version)
   __device__ const Emitter *getEmitter() const { return m_emitter; }

    /// Return a pointer to the BSDF associated with this mesh
   __device__  const BSDF *getBSDF() const { return m_bsdf; }
...
protected:
    ...
    NormalModifier* m_normalModifier = nullptr;

};
#define CallShape(object,function,...) CallCuda2(Shape,shapeType,object,function,const,ETMesh,Mesh,ETSphere,Sphere,__VA_ARGS__)
NORI_NAMESPACE_END
