...
public:
    ...
    __host__ __device__ TVector(Scalar value = (Scalar) 0) { Base::setConstant(value); }

    /// Create a new 2D vector (type error if \c Dimension != 2)
    __host__ __device__ TVector(Scalar x, Scalar y) : Base(x, y) { }

    /// Create a new 3D vector (type error if \c Dimension != 3)
    __host__ __device__ TVector(Scalar x, Scalar y, Scalar z) : Base(x, y, z) { }

    /// Create a new 4D vector (type error if \c Dimension != 4)
    __host__ __device__ TVector(Scalar x, Scalar y, Scalar z, Scalar w) : Base(x, y, z, w) { }
...
public:
    ...
    __host__ __device__ TPoint(Scalar value = (Scalar) 0) { Base::setConstant(value); }

    /// Create a new 2D point (type error if \c Dimension != 2)
    __host__ __device__ TPoint(Scalar x, Scalar y) : Base(x, y) { }

    /// Create a new 3D point (type error if \c Dimension != 3)
    __host__ __device__ TPoint(Scalar x, Scalar y, Scalar z) : Base(x, y, z) { }

    /// Create a new 4D point (type error if \c Dimension != 4)
    __host__ __device__ TPoint(Scalar x, Scalar y, Scalar z, Scalar w) : Base(x, y, z, w) { }
    ...
    /// Assign a point from MatrixBase (needed to play nice with Eigen)
     template <typename Derived> TPoint __host__ __device__  &operator=(const Eigen::MatrixBase<Derived>& p) {
        this->Base::operator=(p);
        return *this;
    }
#ifndef __CUDACC__
...
public:
    ...
    __host__ __device__ Normal3f(Scalar value = 0.0f) { Base::setConstant(value); }

    /// Create a new 3D normal
    __host__ __device__ Normal3f(Scalar x, Scalar y, Scalar z) : Base(x, y, z) { }
...
__host__ __device__ extern void coordinateSystem(const Vector3f &a, Vector3f &b, Vector3f &c);
