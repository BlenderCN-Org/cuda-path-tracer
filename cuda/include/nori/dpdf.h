...
public:
    ...
    __host__ void reserve(size_t nEntries) {
    ...
    __host__ float operator[](size_t entry) const {
     ...
     * This assumes that \ref normalize() has previously been called
     */
   __device__ float getNormalization() const {
     ...
     */
    __device__ size_t sample(float sampleValue) const {
        size_t start = 0;
        size_t end = m_cdf.size();
        ...
        size_t index = start;//(size_t) stdmax((ptrdiff_t) 0, entry - m_cdf.begin() - 1);
        return stdmin(stdmax(index-1,0), m_cdf.size()-2);
    }
     ...
     *     The discrete index associated with the sample
     */
    __device__  size_t sample(float sampleValue, float &pdf) const {
     ...
     *     The discrete index associated with the sample
     */
    __device__ size_t sampleReuse(float &sampleValue) const {
        size_t index = sample(sampleValue);
        //float before = sampleValue;
     ...
     *     The discrete index associated with the sample
     */
   __device__ size_t sampleReuse(float &sampleValue, float &pdf) const {
